...
# 10-shot Hindi examples (Input -> Output) as provided by you
HINDI_FEWSHOT = [
...
def build_fewshot(lang: str):
    """
    Returns a list of (role, content) messages for few-shot prompting.
    For now, only Hindi has provided 10-shot examples. Other languages: zero-shot.
    """
    messages = [{"role": "system", "content": SYSTEM_PROMPT}]

    if lang == "hindi":
        # Interleave as user/assistant pairs
        for src, tgt in HINDI_FEWSHOT:
            messages.append({"role": "user", "content": f"INPUT:\n{src}\n\nOUTPUT:"})
            messages.append({"role": "assistant", "content": tgt})
    
    elif lang == "bangla":
        for src, tgt in BANGLA_FEWSHOT:
            messages.append({"role": "user", "content": f"INPUT:\n{src}\n\nOUTPUT:"})
            messages.append({"role": "assistant", "content": tgt})
    elif lang == "malayalam":
        for src, tgt in MALAYALAM_FEWSHOT:
            messages.append({"role": "user", "content": f"INPUT:\n{src}\n\nOUTPUT:"})
            messages.append({"role": "assistant", "content": tgt})

    elif lang == "tamil":
        for src, tgt in TAMIL_FEWSHOT:
            messages.append({"role": "user", "content": f"INPUT:\n{src}\n\nOUTPUT:"})
            messages.append({"role": "assistant", "content": tgt})
    elif lang == "telugu":
        for src, tgt in TELUGU_FEWSHOT:
            messages.append({"role": "user", "content": f"INPUT:\n{src}\n\nOUTPUT:"})
            messages.append({"role": "assistant", "content": tgt})
...
def correct_sentence(client, base_messages, sent: str, model: str = "gpt-4.1-mini", temperature: float = 0.2) -> str:
    # Compose a new message with the current input
    messages = list(base_messages) + [{"role": "user", "content": f"INPUT:\n{sent}\n\nOUTPUT:"}]
    resp = client.responses.create(
        model=model,
        temperature=temperature,
        input=messages   # ✅ FIX: pass list of messages directly
    )
    try:
...
def main():
    parser = argparse.ArgumentParser(description="GEC with GPT-4 for Indian languages (dev.csv -> predictions.csv)")
    ...
    out_csv = os.path.join(lang_dir, "predictions.csv")

    if not os.path.isfile(dev_csv):
        print(f"[ERROR] Missing file: {dev_csv}", file=sys.stderr)
        sys.exit(1)

    # Load dev data (must have 'Input sentence' and 'Output sentence' columns)
    ...
    # Build few-shot messages for the selected language
    base_messages = build_fewshot(lang)

    inputs = df["Input sentence"].fillna("").astype(str).tolist()
    preds = []

    print(f"[INFO] Running GEC inference on {len(inputs)} sentences for '{lang}'...")
